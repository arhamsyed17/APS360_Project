...
class galaxy_model(nn.Module):
  def __init__(self, img_size=125, out1_channels=20, out2_channels=30, out3_channels=60, out4_channels = 100,kernal_size=5, lastOutput = 10):
    ...
    self.conv4= nn.Conv2d(out3_channels, out4_channels, kernal_size)
    n = self._n_calc(n, kernal_size) # calculate intermediate output dimensions
    print('\nConv4 Output Size:', np.floor(n))
    print('Number of Parameters from Conv4:', self.numParamCNN(out3_channels,out4_channels,kernal_size))
    suM +=  self.numParamCNN(out3_channels,out4_channels,kernal_size)
    
    self.pool4 = nn.AvgPool2d(2,2)
    n = self._n_calc(n, 2, stride=2) # calculate intermediate output dimensions
    n = int(n)
    print('\nPool4 Output Size:', np.floor(n))
    
    
    # Fully connected layers 
    self.fc1 = nn.Linear(out4_channels * n * n, lastOutput)
    print('\nFc1 Output Size:', lastOutput)
    print('Number of Parameters from Fc1:', self.numParamANN(out4_channels*n*n,lastOutput))
    suM +=  self.numParamANN(out4_channels*n*n,lastOutput)
